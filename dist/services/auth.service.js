...
export const selectClinic = async (userId, clinicId, ip, device) => {
    const user = await prisma.user.findUnique({ where: { id: userId } });
...
export const impersonate = async (superAdminId, targetUserId, ip, device) => {
    ...
    if (targetUser.role === 'RECEPTIONIST') {
        ...
        if (staffRoles.includes('ADMIN'))
            targetRole = 'ADMIN';
        else if (staffRoles.includes('DOCTOR'))
            targetRole = 'DOCTOR';
    }
    const token = signToken({
        id: targetUser.id,
...
export const impersonateClinic = async (superAdminId, clinicId, ip, device) => {
    // 1. Verify source is Super Admin
    const superAdmin = await prisma.user.findUnique({ where: { id: superAdminId } });
    if (!superAdmin || superAdmin.role !== 'SUPER_ADMIN') {
