...
export const getClinicStats = async (clinicId) => {
    ...
    const enabledModulesCount = activeModules?.modules
        ? Object.values(typeof activeModules.modules === 'string' ? JSON.parse(activeModules.modules) : activeModules.modules).filter(Boolean).length
        : 0;
    return {
...
export const getClinicActivities = async (clinicId) => {
...
export const deleteClinicStaff = async (clinicId, staffId) => {
    const staff = await prisma.clinicstaff.findUnique({
        where: { id: staffId }
    });
    if (!staff)
...
export const createFormTemplate = async (clinicId, data) => {
...
export const getBookingConfig = async (clinicId) => {
...
export const updateBookingConfig = async (clinicId, config) => {
